...
def get_companies(
    ...
    return company_service.get_all_companies(db, search, sort_by, order, page, page_size)
...
def get_company(
    company_id: int,
    db: Session = Depends(get_db),
    current_user = Depends(get_current_user)  # Allow both admin and regular users to access this endpoint
):
